    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center bg-slate-50/50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-medium text-slate-500 mt-2">Loading profile...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center bg-slate-50 px-4">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-red-100 max-w-md w-full text-center">
          ...
          <Button onClick={() => window.location.reload()} fullWidth className="bg-slate-900 hover:bg-slate-800">
            Refresh Connection
          </Button>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col lg:flex-row gap-8 mt-8">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="px-6 pb-6 flex flex-col items-center text-center -mt-12">
                ...
                <div className="relative">
                  ...
                  <div className={`absolute bottom-1 right-1 w-5 h-5 rounded-full border-2 border-white flex items-center justify-center ${isActive ? 'bg-green-500' : 'bg-amber-500'}`}>
                    {isActive ? <CheckCircle2 className="w-3 h-3 text-white" /> : <Clock className="w-3 h-3 text-white" />}
                  </div>
                </div>
              ...
              <div className="border-t border-slate-100 divide-y divide-slate-100">
                <div className="p-4 flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-slate-900">
                    {user?.createdAt ? new Date(user.createdAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : "May 2026"}
                  </span>
                </div>
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-md uppercase tracking-wide ${user?.isKycVerified !== false ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}>
                      {user?.isKycVerified !== false ? "Verified" : "Pending"}
                    </span>
                  </div>
                  <div className="w-full bg-slate-100 rounded-full h-1.5">
                    <div className={`h-full rounded-full ${user?.isKycVerified !== false ? "bg-green-500 w-full" : "bg-amber-500 w-1/2"}`} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="w-full lg:w-2/3 space-y-6">
            ...
            {/* Company Details */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Company Details</h3>
              <div className="space-y-1">
...
function DetailRow({ icon, label, value, capitalize }) {
  return (
    <div className="flex items-center justify-between py-3 px-2 rounded-lg hover:bg-slate-50 transition-colors">
      ...
      <span className={`text-sm font-semibold text-slate-900 text-right max-w-[200px] sm:max-w-[300px] truncate ${capitalize ? 'capitalize' : ''}`}>
        {value ? value : <span className="text-slate-400 font-normal italic">Not set</span>}
      </span>
    </div>
  );
}